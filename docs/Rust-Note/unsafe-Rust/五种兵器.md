...
// 获取字符串的内存地址和长度fn get_memory_location() -> (usize, usize) {
...
// 在指定的内存地址读取字符串fn get_str_at_location(pointer: usize, length: usize) -> &'static str {
...
// 如果大家想知道为何处理裸指针需要 `unsafe`，可以试着反注释以下代码// let message = get_str_at_location(1000, 10);
}
```
...
详见[静态变量](../Global%20Variables%201d5b9c07c312809cb7a1eca36b2755ee.md)

# 实现unsafe特征

说实话，`unsafe` 的特征确实不多见，如果大家还记得的话，我们在之前的 [Send 和 Sync](https://course.rs/advance/concurrency-with-threads/send-sync.html#%E4%B8%BA%E8%A3%B8%E6%8C%87%E9%92%88%E5%AE%9E%E7%8E%B0sync) 章节中实现过 `unsafe` 特征 `Send`。
...
```rust
#[repr(C)] //指定类型内存布局与 C 语言兼容的属性，核心作用是消除 Rust 与 C 之间的内存布局差异，确保跨语言交互（如 FFI）的正确性。
union MyUnion {
    f1: u32,
    f2: f32,
}
...
```rust
/* automatically generated by rust-bindgen 0.99.9 */#[repr(C)]
pub struct Doggo {

...
let s1 = Some("some1");
    ...
    assert_eq!(s1.or_else(fn_none), s1);// Some1 or_else None = Some1
    assert_eq!(n.or_else(fn_some), s2);// None or_else Some2 = Some2
...
- **`map_or` 逻辑**：如果 `Result` 是 `Ok(v)`，则执行闭包 `fn_closure(v)`；如果是 `Err(_)`，则返回默认值 `V_DEFAULT`。这里 `s` 是 `Ok(10)`，闭包执行 `10 + 2 = 12`，断言成立。
...
fn main() {
    ...
    assert_eq!(s.ok_or(ERR_DEFAULT), o);// Some(T) -> Ok(T)assert_eq!(n.ok_or(ERR_DEFAULT), e);// None -> Err(default)
}
```
...
// AppError 是自定义错误类型，它可以是当前包中定义的任何类型，在这里为了简化，我们使用了单元结构体作为例子。// 为 AppError 自动派生 Debug 特征#[derive(Debug)]
struct AppError;

// 为 AppError 实现 std::fmt::Display 特征impl fmt::Display for AppError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "An Error Occurred, Please Try Again!")// user-facing output
    }
}

// 一个示例函数用于产生 AppError 错误fn produce_error() -> Result<(), AppError> {
    Err(AppError)
}
...
// 根据错误码显示不同的错误信息impl fmt::Display for AppError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
...
- 豆包对这段代码的解释：
    - **设计意图**：封装业务逻辑中的错误，区分「错误码」（用于分类）和「原始信息」（用于定位问题）。
    - **字段类型**：
        - `code: usize`：无符号整数，灵活表示业务场景（如 404 表示资源不存在）。
        - `message: String`：存储具体错误详情，避免在 `Display` 中暴露技术细节（如文件路径）。
    - **`fmt::Display`（用户友好的错误信息）**
        - **作用**：定义错误的「用户可见形态」，例如在终端或前端显示的提示。
        - **设计细节**：
            - **忽略 `message` 字段**：避免向用户暴露技术细节（如 `Page not found` 是内部信息）。
            - **错误码分支**：通过 `match` 对特定错误码（如 404）定制文案，提升用户体验。
            - **使用场景**：`println!("{}", e)` 或日志中的用户日志。
    - **`fmt::Debug`（开发者调试信息）**
        - **作用**：定义错误的「调试形态」，包含所有原始字段，方便开发者定位问题。
        - **设计细节**：
            - **暴露全部字段**：`code` 和 `message` 均显示，用于调试或详细日志。
            - **格式规范**：遵循 Rust 约定的 `Debug` 格式（`结构体名 { 字段: 值 }`）。
            - **使用场景**：`println!("{:?}", e)` 或开发者工具输出。
...
// 实现 From<io::Error> 意味着我们可以将 io::Error 错误转换成自定义的 AppError 错误impl From<io::Error> for AppError {
...
// --------------- 上述代码运行后的可能输出 ---------------// 01. 若 hello_world.txt 文件不存在
Error: AppError { kind: "io", message: "No such file or directory (os error 2)" }
...
[`error-chain`](https://github.com/rust-lang-deprecated/error-chain) 也是简单好用的库，可惜**不再维护**了，但是我觉得它依然可以在合适的地方大放光彩，值得大家去了解下。
